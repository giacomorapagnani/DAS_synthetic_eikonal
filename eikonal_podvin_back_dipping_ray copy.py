...
cx=int(x_max/2)
cz=int(z_max/2)
r=int(6000) #earth's radious
mask_tt= (xoff[np.newaxis,:]-cx)**2 + (zoff[:,np.newaxis]-cz)**2 >= (r+spacing)**2
tt_mat[mask_tt]=np.inf
...
v_min=2000
v_max=15000
vel=v_min + np.arange(n_layers) * (v_max-v_min)/(n_layers-1)                           #dim(vel) != dim(depth)-1

print(vel)

r_min=0
r_max=int(max(cz,cx))
rr=r_min + np.arange(n_layers+1) * (r_max-r_min)/(n_layers)                                       #max depth != zmax

print(rr)

vel_mat=vel_grid_homo(xoff.size,zoff.size,0)
vel_mat=grid_border(vel_mat)
x=np.append(xoff,np.max(xoff))
z=np.append(zoff,np.max(zoff))
for i in range(len(vel)):
    mask_min= (x[np.newaxis,:]-cx)**2 + (z[:,np.newaxis]-cz)**2 >= rr[i]**2
    mask_max= (x[np.newaxis,:]-cx)**2 + (z[:,np.newaxis]-cz)**2 < rr[i+1]**2
    mask_vel=mask_max*mask_min
    vel_mat[mask_vel]=vel[-i]

sl_mat_bord=vel_mat.copy()
sl_mat_bord[mask_tt]=1/vel_mat[mask_tt]

#SOURCE POSITION
sz=1000. #borders != xoff
sx=6000. #borders != zoff
