...
for source_z in range(1,total_shots+1):                                     #CHANGE
    for source_x in range(1,total_shots+1):                                 #CHANGE
        print('shot number : ',shot_number, ' out of : ',total_shots**2)
        start = time.time()

        ########################################################################## FORGE VEL MODEL
        layer=np.load('/home/giacomo/Desktop/DAS_CLASS/DAS/forge_vel_mod/forge_depth_50.npy')
        vp=np.load('/home/giacomo/Desktop/DAS_CLASS/DAS/forge_vel_mod/forge_vp_50.npy')[:-1]
        vs=np.load('/home/giacomo/Desktop/DAS_CLASS/DAS/forge_vel_mod/forge_vs_50.npy')[:-1]

        spacing=5 #grid spacing 10m                                          #CHANGE
        ...
        ############dipping VEL
        #vel=np.array    ((2000.,3000.,4000.)) #dim(vel) != dim(depth)+1
        ...
        #SOURCE POSITION
        sz= source_z * 500.0 #borders != xoff                                   #CHANGE
        sx= source_x * 500.0 #borders != zoff                                   #CHANGE
        ...
        ##################################################################################### SAVE
        np.save('VS_source_tt_mat_forge_' + str(int(sz)) + '_' + str(int(sx)),tt_mat)                       #CHANGE
        np.save('VS_source_wave_front_forge_' + str(int(sz)) + '_' + str(int(sx)),wave_front)               #CHANGE
        #####################################################################################
